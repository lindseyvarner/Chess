...
public class MovePanel extends JPanel {
    ...
    MovePanel() {
        ...
        scrollPane.setPreferredSize(MOVE_PANEL_DIMENSION);
    ...
    public void redo(final Board board, final Table.MoveLog moveLog) {
        ...
        for (final Move move : moveLog.getMoves()) {
            final String moveName = move.toString();

            if (move.getMovedPiece().getPieceAlliance().isWhite()) {
                this.model.setValueAt(moveName, currentRow, 0);
            }
            else if (move.getMovedPiece().getPieceAlliance().isBlack()) {
                this.model.setValueAt(moveName, currentRow, 1);
                currentRow++;
            }
        }
        if (moveLog.getMoves().size() > 0) {
            final Move priorMove = moveLog.getMoves().get(moveLog.size() - 1);
